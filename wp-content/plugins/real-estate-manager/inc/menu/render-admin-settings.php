<?php
    ...
    switch ($field['type']) {
        ...
        case 'textarea':
            $custom_fields = get_option( 'rem_property_fields' );
            $help = $field['help'];
            $disabled = '';
            if (($custom_fields != '') && ($field['name'] == 'property_type_options' || $field['name'] == 'property_purpose_options' || $field['name'] == 'property_status_options')) {
                $disabled = 'disabled';
                $help = '<span class="text-danger">'.__( 'We noticed you are using custom fields, so please set above options', 'real-estate-manager' ).' <a href="'.admin_url( 'edit.php?post_type=rem_property&page=rem_custom_fields' ).'">'.__( 'here', 'real-estate-manager' ).'</a></span>' ;
            }
            ...
            <div class="form-group wrap_<?php echo $field['name']; ?>">
                <label for="<?php echo $field['name']; ?>" class="col-sm-4 control-label"><?php echo $field['title']; ?></label>
                <div class="col-sm-8">
                    <textarea <?php echo $disabled; ?> name="<?php echo $field['name']; ?>" class="form-control" id="<?php echo $field['name']; ?>"><?php echo stripcslashes($field_value); ?></textarea>
            ...
            <div class="form-group wrap_<?php echo $field['name']; ?>">
                ...
                <div class="col-sm-8">
                    <select name="<?php echo $field['name']; ?>" id="<?php echo $field['name']; ?>" class="form-control input-sm">
                        <?php
                        echo '<option value="" '.$selected.' '.$disabled.'>'.__( 'Default', 'real-estate-manager' ).'</option>';
